                ...
                <div class="col-md-9">
                    <div class="page-prehead">
                        <div class="breadcrumb">
                            <NuxtLink class="breadcrumb__item" to="/entitats">Entitats</NuxtLink>
                            <NuxtLink class="breadcrumb__item" :to="`/entitats/${id}`">{{ data.name }}</NuxtLink>
                    ...
                    <div class="row">
                        <div class="col-md-6 col-lg-4">
                            <div class="form-group">
                                <label class="form-label" for="user">Selecciona una persona referent</label>
                                <multiselect
                                    ...
                                    placeholder="Persona referent de l'entitat"
                                    label="name"
                                    track-by="id"
                                    :allow-empty="false"
...
const data = await useApiService('organizations').fetchById(id);
const isOwner = user.value.id == data.owner.id;

if (!(hasAdminRole.value || isOwner)) {
    nuxtApp.$router.push('/entitats');
}
...
const saveForm = async (props, stopLoading) => {
    ...
    if (!v$.value.$invalid) {
        try {
            await useApiService('organizations').update(id, {
                name: name.value,
            ...
            $Snackbar.show({
                text: 'Entitat editat amb éxit',
                pos: 'bottom-center',
                type: 'success',
                duration: 3000,
            });

            nuxtApp.$router.push(`/entitats/${id}`);
        } catch (error) {
            $Snackbar.show({
                text: 'Hi ha hagut un error al crear una entitat. Fes un intent més tard',
                pos: 'bottom-center',
                type: 'error',
                duration: 3000,
            });
