...
<script setup>
    ...
    const { user, hasAdminRole } = storeToRefs(userStore);
    ...
    const users = ref([]);
    const isPublic = ref(originalData.isPublic);
    ...
    const saveForm = async (props, stopLoading) => {
        ...
        if (!v$.value.$invalid) {
            ...
            try {
                await useApiService('discussions').update(id, {
                    title: title.value,
                ...
                $Snackbar.show({
                    text: 'Espai de debat editat amb éxit',
                    pos: 'bottom-center',
                    type: 'success',
                    duration: 3000,
                });

                nuxtApp.$router.push(`/debats/${id}`);
            } catch (error) {
                $Snackbar.show({
    ...
    const showModal = (props, callback) => {
        callback();
        nuxtApp.$showBootstrapModal('DiscussionConfirmChanges');
    }
