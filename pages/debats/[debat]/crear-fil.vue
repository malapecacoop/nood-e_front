                ...
                <div class="col-md-9">
                    <div class="page-prehead">
                        <div class="breadcrumb">
                            ...
                            <a class="breadcrumb__item" href="#">{{ data.title }}</a>
                            <span class="breadcrumb__item active">Creació de fil</span>
                        </div>
                    </div>
...
const data = await useApiService('discussions').fetchById(discussionId);
...
const saveForm = async (props, stopLoading) => {
    ...
    if (!v$.value.$invalid) {
        try {
            await useApiService(`discussions/${discussionId}`).create({
                title: title.value,
            ...
            nuxtApp.$router.push(`/debats/${discussionId}`);
        } catch (error) {
            $Snackbar.show({
